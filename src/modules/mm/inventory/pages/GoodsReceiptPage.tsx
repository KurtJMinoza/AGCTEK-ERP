...
const ROUTE = '/modules/mm/inventory-management/goods-receipt'
...
function supplierLabel(gr: GoodsReceipt) {
    return gr.supplier?.supplierName ?? gr.supplier?.name ?? '—'
}

function materialLabel(line: GoodsReceipt['lines'][number]) {
    return (
...
function hasDiscrepancy(gr: GoodsReceipt) {
    return (gr.lines ?? []).some((l) => !!l.discrepancyFlag)
}
...
const GoodsReceiptPage = () => {
    ...
    const fetchList = useCallback(async () => {
        setLoading(true)
        try {
            const res = await goodsReceiptService.list(queryParams)
            setRows(res.data)
            setTotal(res.total)
    ...
    const [createForm, setCreateForm] = useState({ companyId: '', warehouseId: '', postingDate: '', documentDate: '', stockStatus: 'UNRESTRICTED', remarks: '' })
    const [createLines, setCreateLines] = useState<LineInput[]>([{ materialId: '', quantity: 1, uomId: '', unitCost: 0 }])
    const [creating, setCreating] = useState(false)

    const openCreate = useCallback(() => {
        setCreateForm({ companyId: '', warehouseId: '', postingDate: new Date().toISOString().slice(0, 10), documentDate: new Date().toISOString().slice(0, 10), stockStatus: 'UNRESTRICTED', remarks: '' })
    ...
    const handleCreate = useCallback(async () => {
        ...
        try {
            ...
            pushToast('success', 'Created', 'Goods receipt created.')
            setCreateOpen(false)
            fetchList()
        } catch (err: any) {
    ...
    const refreshDetail = useCallback(async () => {
        if (!detail) return
        try { setDetail(await goodsReceiptService.get(detail.id)) } catch { /* ignore */ }
    }, [detail])

    const handleAction = useCallback(async (action: 'post' | 'cancel' | 'reverse') => {
        if (!detail) return
        try {
            const fn = action === 'post' ? goodsReceiptService.post : action === 'cancel' ? goodsReceiptService.cancel : goodsReceiptService.reverse
            await fn(detail.id)
    ...
    const detailLineColumns = useMemo<ColumnDef<GoodsReceipt['lines'][number]>[]>(() => [
        {
            header: 'Material',
        ...
        { header: 'Quantity', accessorKey: 'quantity', size: 72 },
        {
            ...
            size: 72,
            cell: ({ row }) => Number(row.original.damagedQuantity ?? 0),
        },
        {
            header: 'Variance',
            accessorKey: 'discrepancyFlag',
            size: 88,
            cell: ({ row }) =>
                row.original.discrepancyFlag ? (
                    <Tag className="border-0 bg-amber-100 text-amber-800 dark:bg-amber-500/20 dark:text-amber-200">
                        {row.original.discrepancyFlag}
                    </Tag>
                ) : (
                    '—'
        ...
        },
        {
            ...
            size: 88,
            cell: ({ row }) => fmtMoney(row.original.unitCost),
        },
        {
            header: 'Total Cost',
            accessorKey: 'totalCost',
            size: 88,
            cell: ({ row }) => fmtMoney(row.original.totalCost),
        },
    ], [])
    ...
    const columns: ColumnDef<GoodsReceipt>[] = useMemo(() => [
        ...
        { header: 'Supplier', accessorKey: 'supplier.supplierName', cell: ({ row }) => supplierLabel(row.original) },
        ...
        { header: 'Stock Status', accessorKey: 'stockStatus' },
    ...
    return (
        <PageContainer>
            ...
            <PageHeader
                title="Goods Receipt"
                description="Receive materials into inventory"
                actions={<Button variant="solid" icon={<HiOutlinePlus />} onClick={openCreate}>New Receipt</Button>}
            />
            ...
            >
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    ...
                    <FormItem label="Stock Status">
                        <Select options={[{ value: 'UNRESTRICTED', label: 'Unrestricted' }, { value: 'QUALITY_INSPECTION', label: 'Quality Inspection' }]} value={{ value: createForm.stockStatus, label: createForm.stockStatus }} onChange={(opt: any) => setCreateForm((p) => ({ ...p, stockStatus: opt?.value ?? 'UNRESTRICTED' }))} />
                    </FormItem>
            ...
            <FormDialog
                ...
                size="xl"
                title={detail?.documentNumber ?? 'Goods Receipt'}
                icon={<HiOutlineInboxIn />}
                ...
                {detail && (
                    <div className="space-y-5">
                        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                            ...
                            <InfoCard label="Stock Status" value={detail.stockStatus} />

...
const ROUTE = '/modules/mm/returns-disposal/disposal'
const DISPOSAL_TYPE = 'DISPOSAL'
...
const DisposalPage = () => {
    ...
    const [config, setConfig] = useState<ReturnsDisposalConfig | null>(null)
    const [configForm, setConfigForm] = useState({ amountThreshold: 10000, quantityThreshold: 0 })
    const [savingConfig, setSavingConfig] = useState(false)

    const openConfig = useCallback(async () => {
        setConfigOpen(true)
        try {
            const cfg = await returnsDisposalService.getConfig(warehouses[0]?.companyId || '')
            if (cfg) {
                setConfig(cfg)
                setConfigForm({
                    amountThreshold: Number(cfg.approvalAmountThreshold),
                    quantityThreshold: Number(cfg.approvalQuantityThreshold),
                })
            }
        } catch { /* ignore */ }
    }, [warehouses])
    ...
    const saveConfig = useCallback(async () => {
        ...
        try {
            await returnsDisposalService.upsertConfig({
                companyId: config?.companyId || warehouses[0]?.companyId || '',
                approvalAmountThreshold: configForm.amountThreshold,
                approvalQuantityThreshold: configForm.quantityThreshold,
            })
            pushToast('success', 'Saved', 'Config updated.')
            setConfigOpen(false)
        } catch (err: any) {
            pushToast('danger', 'Error', err?.response?.data?.message || 'Failed')
        } finally { setSavingConfig(false) }
    }, [config, configForm, warehouses])

    /* ── Create ── */
    const [createOpen, setCreateOpen] = useState(false)
    const [createForm, setCreateForm] = useState({ warehouseId: '', reason: 'OBSOLETE', remarks: '' })
    const [createLines, setCreateLines] = useState<LineInput[]>([
        { materialId: '', uomId: '', quantity: 0, unitCost: 0, reason: 'OBSOLETE', stockStatus: 'BLOCKED' },
    ])
    ...
    const handleCreate = useCallback(async () => {
        ...
        try {
            ...
            pushToast('success', 'Created', 'Disposal document created.')
    ...
    const columns: ColumnDef<Disposal>[] = useMemo(() => [
        { header: 'Disposal #', accessorKey: 'disposalNumber' },
    ...
    return (
        <PageContainer>
            ...
            <PageHeader
                title="Disposal"
                description="Disposal documents — permanently remove inventory with accounting events"
                actions={
                    <div className="flex gap-2">
                        <Button variant="plain" icon={<HiOutlineCog />} onClick={openConfig}>Config</Button>
                        <Button variant="solid" icon={<HiOutlinePlus />} onClick={openCreate}>New Disposal</Button>
                    </div>
                }
            />
            ...
            {/* Config Dialog */}
            <FormDialog isOpen={configOpen} onClose={() => setConfigOpen(false)} title="Approval Thresholds" icon={<HiOutlineCog />} footer={<><Button size="sm" onClick={() => setConfigOpen(false)}>Cancel</Button><Button size="sm" variant="solid" loading={savingConfig} onClick={saveConfig}>Save</Button></>}>
                <div className="grid grid-cols-2 gap-4">
                    <FormItem label="Amount Threshold">
                        <Input type="number" value={configForm.amountThreshold} onChange={(e: any) => setConfigForm((p) => ({ ...p, amountThreshold: Number(e.target.value) }))} />
                    </FormItem>
                    <FormItem label="Quantity Threshold (0 = disabled)">
                        <Input type="number" value={configForm.quantityThreshold} onChange={(e: any) => setConfigForm((p) => ({ ...p, quantityThreshold: Number(e.target.value) }))} />
                    </FormItem>
                </div>
            </FormDialog>

            {/* Create */}
            <FormDialog isOpen={createOpen} onClose={() => setCreateOpen(false)} size="lg" title="New Disposal" footer={<><Button size="sm" onClick={() => setCreateOpen(false)}>Cancel</Button><Button size="sm" variant="solid" loading={creating} onClick={handleCreate}>Create</Button></>}>
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    ...
                    <FormItem label="Reason"><Select options={REASON_OPTS} value={REASON_OPTS.find((o) => o.value === createForm.reason)} onChange={(opt: any) => setCreateForm((p) => ({ ...p, reason: opt?.value ?? 'OBSOLETE' }))} /></FormItem>
                </div>
                ...
                {createLines.map((line, idx) => (
                    <div key={idx} className="flex items-end gap-2 mb-2">
                        ...
                        <FormItem label="Reason" className="w-36"><Select size="sm" options={REASON_OPTS} value={REASON_OPTS.find((o) => o.value === line.reason)} onChange={(opt: any) => setCreateLines((p) => p.map((l, i) => i === idx ? { ...l, reason: opt?.value ?? 'OBSOLETE' } : l))} /></FormItem>
                        <Button size="xs" variant="plain" onClick={() => setCreateLines((p) => p.filter((_, i) => i !== idx))}><HiOutlineTrash /></Button>
                    </div>
                ))}
                <Button size="sm" variant="plain" onClick={() => setCreateLines((p) => [...p, { materialId: '', uomId: '', quantity: 0, unitCost: 0, reason: 'OBSOLETE', stockStatus: 'BLOCKED' }])}>+ Add Line</Button>
            </FormDialog>
            ...
            <FormDialog
                ...
                size="xl"
                title={detail?.disposalNumber ?? 'Disposal'}

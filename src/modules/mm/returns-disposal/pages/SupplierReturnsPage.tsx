...
import type { ReturnLine, AuditEntry } from '../types'

const ROUTE = '/modules/mm/returns-disposal/supplier-returns'
...
const SupplierReturnsPage = () => {
    ...
    const fetchList = useCallback(async () => {
        setLoading(true)
        try {
            const res = await returnsDisposalService.listReturns({
                page,
                pageSize,
    ...
    const [createLines, setCreateLines] = useState<LineInput[]>([
        { materialId: '', uomId: '', quantity: 0, unitCost: 0, reason: 'QUALITY_FAILURE', stockStatus: 'BLOCKED' },
    ])
    ...
    const handleCreate = useCallback(async () => {
        ...
        try {
            ...
            await returnsDisposalService.createReturn({
            ...
            pushToast('success', 'Created', 'Supplier return created.')
            setCreateOpen(false)
            fetchList()
        } catch (err: any) {
    ...
    const openDetail = useCallback(async (id: string) => {
        setDetailOpen(true)
        try {
            setDetail(await returnsDisposalService.getReturn(id))
        } catch {
    ...
    const refreshDetail = useCallback(async () => {
        if (!detail) return
        try {
            setDetail(await returnsDisposalService.getReturn(detail.id))
        } catch {
            /* ignore */
        }
    }, [detail])
    ...
    const doAction = useCallback(
        async (label: string, fn: () => Promise<any>) => {
            try {
                await fn()
                pushToast('success', label, `${detail?.returnNumber} — ${label}.`)
                await refreshDetail()
                fetchList()
            } catch (err: any) {
                pushToast('danger', 'Error', err?.response?.data?.message || `${label} failed`)
            }
        },
    ...
    const detailLineColumns = useMemo<ColumnDef<ReturnLine>[]>(() => [
        {
            header: 'Material',
    ...
    const columns: ColumnDef<SupplierReturn>[] = useMemo(
        () => [
            { header: 'Return #', accessorKey: 'returnNumber' },
            { header: 'Supplier', accessorKey: 'supplier.supplierName', cell: ({ row }) => row.original.supplier?.supplierName ?? '—' },
            ...
            { header: 'Created', accessorKey: 'createdAt', cell: ({ row }) => fmtDate(row.original.createdAt) },
            {
                ...
                cell: ({ row }) => (
                    <Dropdown renderTitle={<EllipsisButton />} placement="bottom-end">
                        <Dropdown.Item eventKey="view" onClick={() => openDetail(row.original.id)}>
                            <HiOutlineEye className="mr-2" /> View
                        </Dropdown.Item>
                        {row.original.status === 'DRAFT' && (
                            <Dropdown.Item eventKey="submit" onClick={() => setConfirmAction({ action: 'Submit', fn: async () => { await returnsDisposalService.submitReturn(row.original.id); fetchList() } })}>
                                <HiOutlineCheck className="mr-2" /> Submit
                            </Dropdown.Item>
                        )}
                        {row.original.status === 'PENDING_APPROVAL' && (
                            <Dropdown.Item eventKey="approve" onClick={() => setConfirmAction({ action: 'Approve', fn: async () => { await returnsDisposalService.approveReturn(row.original.id); fetchList() } })}>
                                <HiOutlineCheck className="mr-2" /> Approve
                            </Dropdown.Item>
                        )}
                        {row.original.status === 'PENDING_APPROVAL' && (
                            <Dropdown.Item eventKey="reject" onClick={() => setConfirmAction({ action: 'Reject', fn: async () => { await returnsDisposalService.rejectReturn(row.original.id); fetchList() } })}>
                                <HiOutlineX className="mr-2" /> Reject
                            </Dropdown.Item>
                        )}
                        {row.original.status === 'APPROVED' && (
                            <Dropdown.Item eventKey="ship" onClick={() => setConfirmAction({ action: 'Ship', fn: async () => { await returnsDisposalService.shipReturn(row.original.id); fetchList() } })}>
                                <HiOutlineTruck className="mr-2" /> Ship
                            </Dropdown.Item>
                        )}
                        {['DRAFT', 'REJECTED'].includes(row.original.status) && (
                            <Dropdown.Item eventKey="cancel" onClick={() => setConfirmAction({ action: 'Cancel', fn: async () => { await returnsDisposalService.cancelReturn(row.original.id); fetchList() } })}>
    ...
    return (
        <PageContainer>
            ...
            <PageHeader
                ...
                description="Manage supplier return requests, approvals, and shipments"
                actions={
            ...
            <AdaptiveCard>
                <div className="flex flex-wrap items-center gap-4 mb-4">
                    <Input prefix={<HiOutlineSearch />} placeholder="Search return #..." value={search} onChange={(e: any) => { setSearch(e.target.value); setPage(1) }} className="max-w-xs" />
                </div>
            ...
            >
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <FormItem label="Supplier">
                        <Select options={supplierOpts} value={supplierOpts.find((o) => o.value === createForm.supplierId)} onChange={(opt: any) => setCreateForm((p) => ({ ...p, supplierId: opt?.value ?? '' }))} />
                    </FormItem>
                    ...
                    <FormItem label="Reason">
                        <Select options={REASON_OPTS} value={REASON_OPTS.find((o) => o.value === createForm.reason)} onChange={(opt: any) => setCreateForm((p) => ({ ...p, reason: opt?.value ?? 'QUALITY_FAILURE' }))} />
                    </FormItem>
                </div>
                ...
                {createLines.map((line, idx) => (
                    <div key={idx} className="flex items-end gap-2 mb-2">
                        ...
                        <FormItem label="Reason" className="w-36">
                            <Select size="sm" options={REASON_OPTS} value={REASON_OPTS.find((o) => o.value === line.reason)} onChange={(opt: any) => setCreateLines((p) => p.map((l, i) => i === idx ? { ...l, reason: opt?.value ?? 'QUALITY_FAILURE' } : l))} />
                        </FormItem>
                        <Button size="xs" variant="plain" onClick={() => setCreateLines((p) => p.filter((_, i) => i !== idx))}>
                            <HiOutlineTrash />
                        </Button>
                    </div>
                ))}
                <Button size="sm" variant="plain" onClick={() => setCreateLines((p) => [...p, { materialId: '', uomId: '', quantity: 0, unitCost: 0, reason: 'QUALITY_FAILURE', stockStatus: 'BLOCKED' }])}>
                    + Add Line
                </Button>
            </FormDialog>
            ...
            <FormDialog
                ...
                size="xl"
                title={detail?.returnNumber ?? 'Supplier Return'}
                icon={<HiOutlineTruck />}
                    ...
                    detail ? (
                        <>
                            <div className="flex flex-wrap items-center gap-2">
                                {detail.status === 'DRAFT' && <Button variant="solid" size="sm" onClick={() => doAction('Submitted', () => returnsDisposalService.submitReturn(detail.id))}>Submit</Button>}
                                {detail.status === 'PENDING_APPROVAL' && <Button variant="solid" size="sm" onClick={() => doAction('Approved', () => returnsDisposalService.approveReturn(detail.id))}>Approve</Button>}
                                {detail.status === 'PENDING_APPROVAL' && <Button size="sm" onClick={() => doAction('Rejected', () => returnsDisposalService.rejectReturn(detail.id))}>Reject</Button>}
                                {detail.status === 'APPROVED' && <Button variant="solid" size="sm" onClick={() => doAction('Shipped', () => returnsDisposalService.shipReturn(detail.id))}><HiOutlineTruck className="mr-1" /> Ship</Button>}
                                {['DRAFT', 'REJECTED'].includes(detail.status) && <Button size="sm" onClick={() => doAction('Cancelled', () => returnsDisposalService.cancelReturn(detail.id))}>Cancel</Button>}
                                {detail.status === 'SHIPPED' && (
                                    <Button size="sm" variant="plain" onClick={() => doAction('Reversed', () => returnsDisposalService.reverseReturn(detail.id))}>
                ...
                {detail && (
                    <div className="space-y-5">
                        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                            <InfoCard label="Supplier" value={detail.supplier?.supplierName} />
